...
// Chiều dài cột
$activeSheet->getColumnDimension('A')->setWidth(4, 'pt');
...
$activeSheet->getColumnDimension('T')->setWidth(7, 'pt');
...
$sqlTongTonVinh = "SELECT COUNT(nguoihienmau.ID_NguoiHienMau) as tong FROM danhsachtonvinh 
                        INNER JOIN donvi ON donvi.madonvi = danhsachtonvinh.MaDonVi 
                        INNER JOIN nguoihienmau ON nguoihienmau.ID_NguoiHienMau = danhsachtonvinh.ID_NguoiHienMau 
                        INNER JOIN tonvinh on tonvinh.matonvinh = danhsachtonvinh.MaTonVinh 
                        WHERE tonvinh.matonvinh = '" . $matonvinh . "'";
$kqTongTonVinh = mysqli_query($kn, $sqlTongTonVinh) or die("lỗi truy vấn");
$rowTongTonVinh = mysqli_fetch_array($kqTongTonVinh);

$tongTonVinh = "Tổng" . $rowTongTonVinh['Tong'] . " cá nhân";

$activeSheet->setCellValue('A' . $count, $tongTonVinh);
