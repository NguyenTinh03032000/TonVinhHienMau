                ...
                if (isset($_POST['btnThem'])) {
                    if (isset($_POST['mangchon'])) {
                        foreach ($_POST['mangchon'] as $id) {
                            ...
                            $sql_NguoiHienMau = "select * from nguoihienmau where HoTen = '" . $hoten . "' and NgaySinh =  '" . $ngaysinh . "' and NhomMau = '" . $nhommau1 . "'";
                            $kq_NguoiHienMau = mysqli_query($kn, $sql_NguoiHienMau) or die("Lỗi truy vấn");
                            $row_NguoiHienMau = mysqli_fetch_array($kq_NguoiHienMau);

                            $sqlTonVinh = "insert into danhsachtonvinh(ID, MaTonVinh, ID_NguoiHienMau, MucTonVinh, MaDonVi)
                                values ('', '" . $row_NguoiHienMau['ID_NguoiHienMau'] . "', '" . $nghenghiep . "', '5', '" . $madonvi . "')";
                            $kqTonVinh = mysqli_query($kn, $sqlTonVinh) or die("Lỗi truy vấn");

                            // }

                            $sql_delete = "delete from danhsachxulyrieng where ID = '" . $id . "'";
                            $kq_delete = mysqli_query($kn, $sql_delete) or die("Lỗi truy vấn delete");
                        }
                        echo '<meta http-equiv="refresh" content="0">';
