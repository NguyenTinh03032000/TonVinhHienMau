...
$cboDonVi_In = $_POST['cboDonVi_In'];

if($tenchutich == ""){
    echo '<script>alert("Vui lòng điền thông tin tên chủ tịch.");</script>';
} else if ($tennguoilapbieu == ""){
    echo '<script>alert("Vui lòng điền thông tin người lập biểu.");</script>';
} else if ($tenchutich == "" and $tennguoilapbieu == ""){
    echo '<script>alert("Vui lòng điền thông tin người lập biểu và tên chủ tịch.");</script>';
} else{
...
// Chiều dài cột
$activeSheet->getColumnDimension('A')->setWidth(7, 'pt');
...
if ($cboDonVi_In == "null") {
    ...
    $sqldonvi = "SELECT DISTINCT donvi.madonvi, donvi.tendonvi FROM donvi INNER JOIN danhsachtonvinh ON donvi.madonvi = danhsachtonvinh.MaDonVi where danhsachtonvinh.MaTonVinh = '" . $matonvinh . "' and donvi.madonvi = '" . $cboDonVi_In . "' ORDER BY donvi.madonvi ASC";
}
...
while ($rowdonvi = mysqli_fetch_array($kqdonvi)) {
    ...
    // In đậm từ B4
    $activeSheet->getStyle('A' . $count)->applyFromArray($styleArray);
